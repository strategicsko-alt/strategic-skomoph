      ...
      <div style={{ display: 'flex', gap: '1rem', alignItems: 'flex-start' }}>
        ...
        <div>
          ...
          <div style={{ fontSize: '0.875rem', color: 'var(--secondary-foreground)', marginTop: '0.25rem', display: 'flex', gap: '1rem', alignItems: 'center' }}>
            ...
            {user.work_group && <span style={{ fontSize: "0.75rem", backgroundColor: "#fef08a", color: "#854d0e", padding: "0.1rem 0.4rem", borderRadius: "4px" }}>{user.work_group}</span>}
          </div>
        </div>
      </div>
      <div>
      ...
      <Modal isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} title="จัดการสิทธิ์ผู้ใช้งาน">
        <form onSubmit={handleSaveUser}>
          ...
          {editRole.includes('province') && (
            <div style={{ marginBottom: '1.25rem' }}>
              <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: 600, marginBottom: '0.5rem' }}>กลุ่มงาน (Work Group)</label>
              <select
                className="input-field"
                value={editWorkGroup}
